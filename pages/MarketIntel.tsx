...
const MarketIntel = () => {
  ...
  return (
    <div className="space-y-8">
      <header className="nl-card p-6 md:p-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="nl-page-eyebrow text-[#B6FF00]">Marketing e inteligência de mercado</p>
            <h1 className="nl-page-title mt-2">Marketing e Inteligência de Mercado</h1>
            <p className="nl-page-description mt-2">
              Monitore preços, encontre oportunidades e veja quando seus produtos precisam de mais dados de mercado.
            </p>
            {lastRun ? (
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="grid gap-3">
            {loading && !comparisons.length ? (
              <div className="grid min-h-[160px] place-items-center rounded-2xl border border-white/[0.08] bg-white/[0.035] p-6">
                <span className="text-2xl font-black tracking-[0.24em] text-[#B6FF00]">NEXT LEVEL</span>
              </div>
            ) : null}
            {!loading && !comparisons.length ? (
              <div className="rounded-2xl border border-dashed border-white/[0.09] bg-white/[0.025] p-6 text-sm text-zinc-300">
                Nenhum produto com dados de mercado disponível para comparação. Cadastre produtos, conecte canais ou importe dados para iniciar a análise.
              </div>
            ) : null}
            ...
            {comparisons.map((item) => {
              ...
              return (
                <div
                  key={item.productId}
                  className="rounded-2xl border border-white/[0.08] bg-white/[0.04] p-4 shadow-md transition hover:border-lime-400/30"
                >
        ...
        <div className="space-y-4">
          ...
          <div className="nl-card p-5">
            {trends.length === 0 ? (
              <p className="text-sm text-zinc-400">Nenhum sinal externo confirmado ainda. Atualize os sinais quando houver produtos e canais conectados.</p>
            ) : (
              <div className="flex flex-wrap gap-3">
                {trends.map((trend) => {
...
const MetricCard = ({
  ...
  <div className={`nl-card nl-card-interactive relative bg-gradient-to-br p-5 ${tone}`}>
    <div className="flex items-center justify-between">
      <div>
      ...
      <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-white/[0.08] bg-black/30 shadow-inner">
        {icon}
      </div>
    </div>
  </div>
