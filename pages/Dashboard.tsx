              ...
              <>
                <div className="relative h-[236px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={pieData} cx="50%" cy="50%" innerRadius={70} outerRadius={94} paddingAngle={3} dataKey="value" stroke="none">
                        {pieData.map((_, index) => (
                          <Cell key={`operation-${index}`} fill={PIE_COLORS[index % PIE_COLORS.length]} />
                        ))}
                      </Pie>
                      <Tooltip />
                    </PieChart>
                ...
                <div className="mt-5 space-y-3">
                  {pieData.slice(0, 4).map((item, index) => (
                    <div key={item.name} className="flex items-center justify-between border-b border-white/[0.06] pb-3 text-sm last:border-b-0">
                      <span className="flex min-w-0 items-center gap-2 text-[#B7C0BA]">
                        <span className="h-2 w-2 shrink-0 rounded-full" style={{ background: PIE_COLORS[index % PIE_COLORS.length] }} />
                        <span className="truncate">{item.name || ["Vendas", "Atendimento", "Produtos", "Custos"][index]}</span>
                      </span>
                      <span className="font-bold text-[#f5f7f2]">{Number(item.value || 0).toLocaleString("pt-BR", { notation: "compact" })}</span>
                    </div>
                  ))}
                </div>
              </>
