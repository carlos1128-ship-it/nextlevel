...
const Chat = () => {
  ...
  return (
    <div className="flex h-[calc(100dvh-128px)] min-h-[520px] flex-col overflow-hidden rounded-[28px] border border-white/[0.08] bg-[#080D0B] shadow-[0_24px_70px_rgba(0,0,0,0.28)]">
      <header className="border-b border-white/[0.08] bg-[#101418] px-5 py-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="nl-page-eyebrow text-[#B6FF00]">Assistente IA</p>
            <h1 className="mt-1 text-2xl font-black tracking-tight text-zinc-100">Chat estratégico</h1>
            <p className="mt-1 text-sm text-zinc-400">
              Converse com a IA sobre vendas, custos, operação e oportunidades da <span className="text-zinc-200">{companyName}</span>.
            </p>
          </div>
          <button
            onClick={() => {
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {QUICK_PROMPTS.map((prompt) => (
            <button
              ...
              disabled={isTyping}
              className="rounded-full border border-[#B6FF00]/18 bg-[#B6FF00]/8 px-4 py-2 text-xs font-semibold text-zinc-300 transition hover:border-[#B6FF00]/45 hover:text-zinc-100 disabled:opacity-50"
            >
              {prompt}
            </button>
          ))}
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto bg-[radial-gradient(circle_at_top,_rgba(182,255,0,0.07),_transparent_32%),linear-gradient(180deg,#080D0B_0%,#050706_100%)] p-5">
        {safeMessages.length === 0 ? (
          ...
          safeMessages.map((msg) => (
            ...
            >
              {msg.sender === "ai" ? (
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-[#B6FF00]/20 bg-[#B6FF00]/12 text-[#B6FF00]">
              ...
              <div
                className={`max-w-[88%] rounded-3xl p-4 shadow-md md:max-w-[70%] ${msg.sender === "user"
                    ...
                    : "rounded-bl-md border border-white/[0.08] bg-[#171D1A] text-zinc-100"
                  }`}
              >
                <p className="whitespace-pre-wrap break-words text-sm leading-relaxed">{msg.text}</p>
              </div>
              {msg.sender === "user" ? (
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-white/[0.08] bg-[#101418]">
        ...
        {isTyping ? (
          <div className="flex items-start justify-start gap-3">
            ...
            <div className="rounded-2xl border border-white/[0.08] bg-[#171D1A]">
              <TypingIndicator />
            </div>
          </div>
        ) : null}
      ...
      <div className="border-t border-white/[0.08] bg-[#101418] p-4">
        <div className="relative">
          <input
            ...
            placeholder="Pergunte sobre vendas, custos, clientes, produtos ou oportunidades..."
            className="w-full rounded-full border border-white/[0.09] bg-[#181A1D] py-3 pl-5 pr-14 text-zinc-100 transition placeholder:text-zinc-600 focus:border-[#B6FF00]/70 focus:outline-none"
          />
          <button
            type="button"
