...
const NicheModal = ({ selectedNiche, onSelect, onConfirm, loading = false }: NicheModalProps) => {
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 sm:p-6">
      <div className="w-full max-w-5xl overflow-hidden rounded-[36px] border border-white/10 bg-[linear-gradient(180deg,rgba(15,23,42,0.96),rgba(5,8,22,0.98))] shadow-[0_30px_120px_rgba(0,0,0,0.45)]">
        <div className="border-b border-white/10 px-6 py-6 sm:px-8">
          ...
          <h2 className="mt-3 text-3xl font-black tracking-tight text-white sm:text-4xl">
            Qual e o nicho da sua operacao?
          </h2>
          <p className="mt-3 max-w-2xl text-sm leading-7 text-zinc-400 sm:text-base">
            Vamos adaptar atalhos, prioridade do menu e linguagem do painel para o contexto do seu negocio.
          </p>
        </div>

        <div className="grid gap-4 p-6 sm:grid-cols-2 xl:grid-cols-5 sm:p-8">
        ...
        <div className="flex flex-col gap-3 border-t border-white/10 px-6 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-8">
          <p className="text-xs uppercase tracking-[0.18em] text-zinc-500">
            Essa escolha pode ser alterada depois no seu perfil.
          </p>
          <button
            type="button"
            onClick={onConfirm}
